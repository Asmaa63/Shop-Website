...
useEffect(() => {
  if (initialCategory) {
    const formatted = decodeURIComponent(initialCategory);
    ...
    const fetchProducts = async () => {
      ...
      try {
        const res = await fetch(
          "https://68f0b6520b966ad50033e04c.mockapi.io/ecommerce/products"
        );
  ...
  productsList = productsList.filter((p) => {
    const normalize = (str: string) =>
      str.toLowerCase().replace(/[\s&-]/g, ""); // remove spaces, & and -
    ...
    return productsList;
  }, [products, selectedCategory, selectedBrand, sortBy, priceRange, searchQuery]);
...
<div>
  ...
  <div className="px-2">
    <Slider
      ...
      max={10000}
      step={50}
      className="w-full h-2 bg-red-600 rounded"
    />
