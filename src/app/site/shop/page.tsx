...
export default function ShopPage() {
  ...
  const [products, setProducts] = useState<RawProductData[]>([]);
...
useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedBrand !== "All") {
      productsList = productsList.filter((p) => p.brand === selectedBrand);
    }
  ...
  step={50}
  className="
    relative flex items-center w-full h-2 
    bg-blue-100 rounded-full 
    before:content-[''] before:absolute before:h-2 before:bg-blue-500 before:rounded-full
    data-[orientation=horizontal]:before:w-[var(--radix-slider-range-end)] 
  "
>
  <div className="absolute w-full h-2 bg-blue-900 rounded-full" />
  <div className="absolute h-2 bg-blue-500 rounded-full" />
  <div
    className="
      w-5 h-5 rounded-full bg-blue-600 
