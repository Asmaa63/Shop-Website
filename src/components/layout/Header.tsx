...
export default function Navbar() {
	...
	// We assume cart items are stored in state.items and each item has a 'quantity' property.
	const cartItemsCount = useCartStore(state => 
        state.items.reduce((total, item) => total + item.quantity, 0)
    );
    
    // Original line that caused the error (removed): const { getTotalItems } = useCartStore();

	// Use selector for wishlist items
	const wishlistItems = useWishlistStore(state => state.items);

	// Calculate the count directly
	const wishlistCount = wishlistItems.length;
    // Original line that caused the error (removed): const cartItemsCount = getTotalItems();
    // Original line that caused the error (removed): const wishlistCount = wishlistItems.length;
